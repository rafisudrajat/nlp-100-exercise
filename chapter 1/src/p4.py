...
def create_atom_from_sentence(string:str, selected_word_idx:list[int])->Dict[str,int]:
    word_count=0
    word_counter=set()
    ...
    for i in range(len(string)):
        ...
        if (ord(string[i])>=65 and ord(string[i])<=90) or (ord(string[i])>=97 and ord(string[i])<=122):
            ...
            # For every word in the sentence we get the first one or two characters and save it to the dictionary 
